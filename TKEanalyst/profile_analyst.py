...
def vna_file_name2coordinates(file_ending, vna_file_name):
    ...
    xyz_list = vna_file_name.replace("__", "-").strip(file_ending).split("_")[0:3]
...
def get_data_info(file_ending, folder_name="data/test-example"):
...
@log_actions
def process_adv_files(input_file_name):
    ...
    experiment_meta = get_data_info(experiment_setup["file type"], experiment_setup["folder name"])
    target_folder = r"" + experiment_setup["folder name"]
    vna_stats_dict = {}
    vna_stats_dict_despiked = {}
    for vna_fn in experiment_meta["vna files"]:
        print("\n   *** *** processing %s *** ***" % vna_fn)
        vna_df = read_vna(target_folder + "/" + vna_fn)
        print(" * writing raw data to %s " % str(target_folder + "/%s-raw.xlsx" % vna_fn.split(experiment_setup["file type"])[0]))
        vna_df.to_excel(str(target_folder + "/%s-raw.xlsx" % vna_fn.split(experiment_setup["file type"])[0]))
        ...
        vna_stats_dict.update({vna_fn.split(experiment_setup["file type"])[0]: vna_stats})
        ...
        print(" *  Writing spike counts to %s " % str(target_folder + "/%s-spikes.xlsx" % vna_fn.split(experiment_setup["file type"])[0]))
        spike_df.to_excel(target_folder + "/%s-spikes.xlsx" % vna_fn.split(experiment_setup["file type"])[0])
        print(" *  Writing de-spiked data to %s " % str(target_folder + "/%s-despiked.xlsx" % vna_fn.split(experiment_setup["file type"])[0]))
        spike_df.to_excel(target_folder + "/%s-spikes.xlsx" % vna_fn.split(experiment_setup["file type"])[0])
        ...
        vna_stats_dict_despiked.update({vna_fn.split(experiment_setup["file type"])[0]: vna_stats_despiked})
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        process_adv_files(input_file_name=str(sys.argv[1]))
    else:
        print("TKE analyst needs and input.xlsx file!")
